...
from sqlalchemy import CheckConstraint, DateTime, ForeignKey, Identity, Index, Integer, Text, func
...
class Reservation(Base):
    ...
    # RPCN username of the host. Null only on rows from before login existed,
    # which therefore have no owner left.
...
class ReservationComment(Base):
    ...
    author_subject: Mapped[str | None] = mapped_column(Text)
...
class ReservationParticipant(Base):
    ...
    __table_args__ = (
        ...
        # One live seat per account; a cancelled seat does not block rejoining.
        Index(
            "uq_reservation_participants_active_subject", "reservation_id", "subject", unique=True,
            postgresql_where="cancelled_at IS NULL AND subject IS NOT NULL",
        ),
    )
    id: Mapped[int] = mapped_column(Integer, Identity(always=True), primary_key=True)
    reservation_id: Mapped[int] = mapped_column(ForeignKey("reservations.id", ondelete="CASCADE"), nullable=False)
